...
public class ChromaKeyVideoActivity extends AppCompatActivity implements Scene.OnUpdateListener{
  ...
  protected void onCreate(Bundle savedInstanceState) {
   ...
   /* arFragment.setOnTapArPlaneListener(
            ...
            if (image.getTrackingState() == TrackingState.TRACKING) {
                ...
                if (image.getName().equals("image")) {
                   ...
                   //playVideo(image.createAnchor(image.getCenterPose()));
                    ...
                    displayHand(image.createAnchor(image.getCenterPose()));
                    break;
                }
            }
        }
    ...
    private void displayHand(Anchor anchor) {
...
/*
 ViewRenderable.builder().setView(this, R.layout.test_view).build()
                    ...
                    //Collection<Anchor> buttonAnchors = image.getAnchors();


                    AnchorNode anchorNode2 = new AnchorNode(image.createAnchor(image.getCenterPose()));

                    Node buttonNode = new Node();
                    buttonNode.setParent(anchorNode2);

                    buttonNode.setLocalScale(new Vector3(0.05f,0.05f,0.05f));

                    ExternalTexture buttontexture;
                    buttontexture = new ExternalTexture();
                    buttontexture
                            .getSurfaceTexture()
                            .setOnFrameAvailableListener((SurfaceTexture surfaceTexture)->{
                                buttonNode.setRenderable(viewRenderable);
                            });
                                //buttontexture.getSurfaceTexture().setOnFrameAvailableListener(null);
                                //ImageButton button = (ImageButton)findViewById(R.layout.test_view);
